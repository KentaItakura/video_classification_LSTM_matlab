...
% 
% The video classification or recogintion from video will be more intriguing 
% as more video data is being accumulating currentely and we can easily record 
% videos with, for example, smartphones. In this example, running/walking classification 
% was conducted with the video clips taken while running and walking using a deep 
% learning-based technique called LSTM (Long Short Term Memory) which can classifies 
% time-series data. 
% 
% This example was created based on a Mathworks official documentation located 
% at 
% 
% <https://jp.mathworks.com/help/deeplearning/examples/classify-videos-using-deep-learning.html 
% https://jp.mathworks.com/help/deeplearning/examples/classify-videos-using-deep-learning.html>
% 
% While the official example requires down-loading a dataset about 2 GB, this 
% example can try that with a small amout of data, which may help you giving a 
% try easily. Note that this is just an example of LSTM with images and please 
% refer to the official example for your further study.  
%% Load Pretrained Convolutional Network
%% 
% * A pre-trained network, resnet 18 was used for feature extraction here in 
% this example.
% * The features extracted from the pre-trained network were fed into LSTM layer 
% as shown below. 
...
%% Load Data
%% 
% * The video clips for the classification were retrieved from the videos recorded 
% while running and walking which lasted about 5 min and 10 min, respectively. 
% * The 2 kinds of video were taken at the same path to exclude the difference 
% of the scene to capture.
% * In the future example, the scence with various place/moving condition should 
% be prepared.    
...
%% 
% * Image features are extracted with the pre-trained network to feed to the 
% LSTM network with "single type".
% * As the process of feature extraction takes a long time, pls load the variable 
% R and W pre-calculated for you. 
...
% 
%% 
% * A video clip whose duration is from minDuration to maxDuration as difined 
% below was obtained. 
...
% Prepare the data for training by partitioning the data into training and validation 
% partitions.
% 
% *Create Training and Validation Partitions*
% 
% Partition the data. Assign 70% of the data to the training partition and 30% 
...
%% Create LSTM Network
% Create an LSTM network that can classify the sequences of feature vectors 
% representing the videos.
% 
% Define the LSTM network architecture. Specify the following network layers.
%% 
% * A sequence input layer with an input size corresponding to the feature dimension 
% of the feature vectors
% * Here, the dimension of the extracted feature with resnet18 was 512, meaning 
% the numFeatures is 512. 
% * LSTM layer with 1500 hidden units with a dropout layer afterwards. To output 
% only one label for each sequence by setting the |'OutputMode'| option of the 
% BiLSTM layer to |'last'|
% * You may use BiLSTM  layer with which the image sequences can be learned 
% with forward and backward time-series. 
% * 2 LSTM layer can be put in the "layers" which might learn more detailed 
% information of the time-series data. 
% * A fully connected layer with an output size corresponding to the number 
% of classes (here, 2), a softmax layer, and a classification layer.
% * If you would like to esimated a certain value from the time-series data, 
% you can prepare "regressionLayer" with numerical label (data) instead of the 
% categorical "|labelsTrain|". 
% * The dropout layer contribute to prevent the network from being "over-tuned" 
...
%% 
% * Set a mini-batch size 16, an initial learning rate of 0.0001, and a gradient 
% threshold of 2 (to prevent the gradients from exploding).
...
%% 
% * If you would like to plot the data in training process such as the accucary 
% and loss, please check the values saved in the variable |info|. 

[netLSTM,info] = trainNetwork(sequencesTrain,labelsTrain,layers,options);
%% 
% Calculate the classification accuracy of the network on the validation set.  
% If the accuracy is quite satisfactory, please prepare the test video clips to 
% explore the feasibility of this LSTM network. 
...
%% Things to consider 
%% 
% # some clips are difficult to classify with a short period of the movie => 
% the accuracy is likely to increase if the duration at each clip gets longer
% # What if the two LSTM layers are used while this example uses one LSTM layer?
% # What if BiLSTM layer is used?